  ...
  void StereoRefToImplicit(OBMol& mol, OBStereo::Ref atomId) {
    ...
    for (std::vector<OBGenericData*>::iterator data = vdata.begin(); data != vdata.end(); ++data) {
      ...
      if (datatype == OBStereo::CisTrans) {
        ...
        replace_if(ct_cfg.refs.begin(), ct_cfg.refs.end(), [currr] (OBStereo::Ref a) -> bool {
					if (a==currr) return 1; 
					else return 0;		
				}
        ...
        replace_if(ts_cfg.refs.begin(), ts_cfg.refs.end(), [currr] (OBStereo::Ref a) -> bool {
					if (a==currr) return 1; 
					else return 0;		
				}
				, (OBStereo::Ref) OBStereo::ImplicitRef);
      ...
      if (datatype == OBStereo::CisTrans) {
        ...
        if (ct_cfg.begin == centerId || ct_cfg.end == centerId) {
          ...
          if (ct_cfg.begin == centerId)
            replace_if(ct_cfg.refs.begin(), ct_cfg.refs.begin()+2, [currr] (OBStereo::Ref a) -> bool {
					if (a==currr) return 1; 
					else return 0;		
				}
				, (OBStereo::Ref) newId);
          if (ct_cfg.end == centerId)
            replace_if(ct_cfg.refs.begin()+2, ct_cfg.refs.end(), [currr] (OBStereo::Ref a) -> bool {
					if (a==currr) return 1; 
					else return 0;		
				}
				, (OBStereo::Ref) newId);
