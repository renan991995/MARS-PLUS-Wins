...
namespace OpenBabel {
  ...
  double OBForceFieldUFF::E_Bond()
  {
    ...
    for (i = _bondcalculations.begin(); i != _bondcalculations.end(); ++i) {
      ...
      IF_OBFF_LOGLVL_HIGH {
        _snprintf(_logbuf, BUFF_SIZE, "%-5s %-5s  %4.2f%8.3f   %8.3f     %8.3f   %8.3f   %8.3f\n",
    ...
    IF_OBFF_LOGLVL_MEDIUM {
      _snprintf(_logbuf, BUFF_SIZE, "     TOTAL BOND STRETCHING ENERGY = %8.3f %s\n",  energy, GetUnit().c_str());
      OBFFLog(_logbuf);
    }
    return energy;
  }
  ...
  double OBForceFieldUFF::E_Angle()
  {
    ...
    for (i = _anglecalculations.begin(); i != _anglecalculations.end(); ++i) {
      ...
      IF_OBFF_LOGLVL_HIGH {
        _snprintf(_logbuf, BUFF_SIZE, "%-5s %-5s %-5s%8.3f  %8.3f     %8.3f   %8.3f   %8.3f\n", (*i).a->GetType(), (*i).b->GetType(),
    ...
    IF_OBFF_LOGLVL_MEDIUM {
      _snprintf(_logbuf, BUFF_SIZE, "     TOTAL ANGLE BENDING ENERGY = %8.3f %s\n", energy, GetUnit().c_str());
      OBFFLog(_logbuf);
    }
    return energy;
  }
  ...
  double OBForceFieldUFF::E_Torsion()
  {
    ...
    for (i = _torsioncalculations.begin(); i != _torsioncalculations.end(); ++i) {
      ...
      IF_OBFF_LOGLVL_HIGH {
        _snprintf(_logbuf, BUFF_SIZE, "%-5s %-5s %-5s %-5s%6.3f       %8.3f     %8.3f\n",
                 (*i).a->GetType(), (*i).b->GetType(),
    ...
    IF_OBFF_LOGLVL_MEDIUM {
      _snprintf(_logbuf, BUFF_SIZE, "     TOTAL TORSIONAL ENERGY = %8.3f %s\n", energy, GetUnit().c_str());
      OBFFLog(_logbuf);
    }
  ...
  */
  ...
  double OBForceFieldUFF::E_OOP()
  {
    ...
    for (i = _oopcalculations.begin(); i != _oopcalculations.end(); ++i) {
      ...
      IF_OBFF_LOGLVL_HIGH {
        _snprintf(_logbuf, BUFF_SIZE, "%-5s %-5s %-5s %-5s%8.3f   %8.3f     %8.3f\n", (*i).a->GetType(), (*i).b->GetType(), (*i).c->GetType(), (*i).d->GetType(),
    ...
    IF_OBFF_LOGLVL_HIGH {
      _snprintf(_logbuf, BUFF_SIZE, "     TOTAL OUT-OF-PLANE BENDING ENERGY = %8.3f %s\n", energy, GetUnit().c_str());
      OBFFLog(_logbuf);
    }
    return energy;
  }
  ...
  double OBForceFieldUFF::E_VDW()
  {
    ...
    for (i = _vdwcalculations.begin(); i != _vdwcalculations.end(); ++i, ++j) {
      ...
      IF_OBFF_LOGLVL_HIGH {
        _snprintf(_logbuf, BUFF_SIZE, "%-5s %-5s %8.3f  %8.3f  %8.3f\n", (*i).a->GetType(), (*i).b->GetType(),
    ...
    IF_OBFF_LOGLVL_MEDIUM {
      _snprintf(_logbuf, BUFF_SIZE, "     TOTAL VAN DER WAALS ENERGY = %8.3f %s\n", energy, GetUnit().c_str());
      OBFFLog(_logbuf);
    }
  ...
  double OBForceFieldUFF::E_Electrostatic()
  {
    ...
    IF_OBFF_LOGLVL_MEDIUM {
      _snprintf(_logbuf, BUFF_SIZE, "     TOTAL ELECTROSTATIC ENERGY = %8.3f %s\n", energy, GetUnit().c_str());
      OBFFLog(_logbuf);
    }
  ...
  bool OBForceFieldUFF::SetupVDWCalculation(OBAtom *a, OBAtom *b, OBFFVDWCalculationUFF &vdwcalc)
  {
    ...
    if (parameterA == nullptr || parameterB == nullptr) {
      IF_OBFF_LOGLVL_LOW {
        _snprintf(_logbuf, BUFF_SIZE, "    COULD NOT FIND PARAMETERS FOR VDW INTERACTION %d-%d (IDX)...\n",
                 a->GetIdx(), b->GetIdx());
        OBFFLog(_logbuf);
      }
      return false;
  ...
  bool OBForceFieldUFF::SetupCalculations()
  {
    ...
    FOR_ATOMS_OF_MOL(atom, _mol) {
      ...
      if (parameterB == nullptr) {
        _snprintf(_logbuf, BUFF_SIZE, "    COULD NOT FIND PARAMETERS FOR ATOM %d (IDX)...\n",
                 atom->GetIdx());
    ...
    FOR_ANGLES_OF_MOL(angle, _mol) {
      ...
      if (parameterA == nullptr || parameterB == nullptr || parameterC == nullptr) {
        IF_OBFF_LOGLVL_LOW {
          _snprintf(_logbuf, BUFF_SIZE, "    COULD NOT FIND PARAMETERS FOR ANGLE %d-%d-%d (IDX)...\n",
      ...
      if (coordination != parameterB->_ipar[0]) {
        IF_OBFF_LOGLVL_LOW {
          _snprintf(_logbuf, BUFF_SIZE, "    CORRECTED COORDINATION FOR ANGLE %d-%d-%d (IDX)... WAS %d NOW %d\n",
    ...
    FOR_TORSIONS_OF_MOL(t, _mol) {
      ...
      if (parameterB == nullptr || parameterC == nullptr) {
        IF_OBFF_LOGLVL_LOW {
          _snprintf(_logbuf, BUFF_SIZE, "    COULD NOT FIND PARAMETERS FOR TORSION X-%d-%d-X (IDX)...\n",
                   b->GetIdx(), c->GetIdx());
          OBFFLog(_logbuf);
        }
        return false;
  ...
  bool OBForceFieldUFF::SetTypes()
  {
    ...
    IF_OBFF_LOGLVL_LOW {
      ...
      FOR_ATOMS_OF_MOL (a, _mol) {
        _snprintf(_logbuf, BUFF_SIZE, "%d\t%s\t%s\n", a->GetIdx(), a->GetType(),
