...
namespace OpenBabel {
  ...
  bool VASPFormat::ReadMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    if (hasEnthalpy) {
      ...
      _snprintf(tag, BUFF_SIZE, "%f", en_kcal_per_mole);
      enthalpyPD->SetValue(tag);
      _snprintf(tag, BUFF_SIZE, "%f", pv_kcal_per_mole);
      enthalpyPD_pv->SetValue(tag);
      _snprintf(tag, BUFF_SIZE, "%f", enthalpy_eV);
      enthalpyPD_eV->SetValue(tag);
      _snprintf(tag, BUFF_SIZE, "%f", pv_eV);
  ...
  bool VASPFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    if (!mol.HasData(OBGenericDataType::UnitCell)) {
      ...
      for (int ii = 0; ii < 3; ii++) {
        _snprintf(buffer, BUFF_SIZE, "0.0  0.0  0.0");
        ofs << buffer << endl;
      }
    }
    else
    {
      ...
      for (vector<vector3>::const_iterator i = cell.begin();
           i != cell.end(); ++i) {
        _snprintf(buffer, BUFF_SIZE, "%20.15f%20.15f%20.15f",
                 i->x(), i->y(), i->z());
        ofs << buffer << endl;
      }
    }
    ...
    if (!vasp4Format) {
      for (vector< std::pair<int, int> >::const_iterator
        ...
        _snprintf(buffer, BUFF_SIZE, "%-3s ", OBElements::GetSymbol(it->first));
        ofs << buffer ;
      }
      ofs << endl;
    }
    ...
    for (vector< std::pair<int, int> >::const_iterator
           it = atomicNums.begin(),
           it_end = atomicNums.end(); it != it_end; ++it) {
      _snprintf(buffer, BUFF_SIZE, "%-3u ", it->second);
      ofs << buffer ;
    }
    ofs << endl;
    ...
    {
      // Print coordinates
      _snprintf(buffer,BUFF_SIZE, "%26.19f %26.19f %26.19f",
