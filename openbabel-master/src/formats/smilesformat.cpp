                                      ...
                                      std::string &buffer)
  {
          ...
          for(externalBond = externalBonds->begin();externalBond != externalBonds->end();++externalBond) {
            if (externalBond->second.first == atom) {
              ...
              if (bond->GetBondOrder() == 4)
                buffer += '$';
              char tmp[10];
              _snprintf(tmp, 10, "%d", externalBond->first);
              buffer += tmp;
              break;
            }
          }
    ...
    if (options.isomeric && iso) {
      if (iso >= 10000) // max 4 characters
        ...
        _snprintf(iso, 8, "%u", atom->GetIsotope());
        buffer += iso;
      }
    }
    ...
    if (charge != 0) {
      ...
      if (abs(charge) > 1) {
        char tchar[10];
        _snprintf(tchar, 10, "%d", abs(charge));
        buffer += tchar;
      }
    }
      ...
      for (bci = vclose_bonds.begin(); bci != vclose_bonds.end(); ++bci) {
        ...
        if (bci->ringdigit > 9) {
          ...
          if (bci->ringdigit > 99)
            buffer += '(';
          char digits[8];
          _snprintf(digits, 8, "%d", bci->ringdigit);
          buffer += digits;
          if (bci->ringdigit > 99)
            buffer += ')';
        }
    ...
    if(!pConv->IsOption("smilesonly")) {
      ...
      if (pConv->IsOption("x") && pmol->HasData("SMILES Atom Order")) {
        ...
        for (unsigned int i = 0; i < vs.size(); i++) {
          ...
          if (i > 0)
            buffer += ',';
          _snprintf(tmp, 15, "%.4f", atom->GetX());
          buffer += tmp;
          buffer += ',';
          _snprintf(tmp, 15, "%.4f", atom->GetY());
          buffer += tmp;
        }
      }
    ...
    for (j = 0;j < mol.NumConformers();j++)
      {
        ...
        for (unsigned int index = 0; index < canonical_order.size();
          ...
          _snprintf(coords, 100, "%9.3f %9.3f %9.3f", atom->GetX(), atom->GetY(), atom->GetZ());
          ofs << coords << endl;
        }
      }
    return(true);
