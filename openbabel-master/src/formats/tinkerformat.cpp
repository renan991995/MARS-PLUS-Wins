...
namespace OpenBabel
{
  ...
  bool TinkerFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    if (!mmffTypes && !mm3Types && !classTypes) {
      _snprintf(buffer, BUFF_SIZE, "%6d %-20s   MM2 parameters\n",mol.NumAtoms(),mol.GetTitle());
      mm2Types = true;
    }
    else if (mm3Types)
      _snprintf(buffer, BUFF_SIZE, "%6d %-20s   MM3 parameters\n",mol.NumAtoms(),mol.GetTitle());
    else if (classTypes)
      _snprintf(buffer, BUFF_SIZE, "%6d %-20s   Custom parameters\n",mol.NumAtoms(),mol.GetTitle());
    else
      _snprintf(buffer, BUFF_SIZE, "%6d %-20s   MMFF94 parameters\n",mol.NumAtoms(),mol.GetTitle());
    ofs << buffer;
    ...
    for(i = 1;i <= mol.NumAtoms(); i++)
      {
        ...
        _snprintf(buffer, BUFF_SIZE, "%6d %2s  %12.6f%12.6f%12.6f %5d",
                 i,
        ...
        for (bond = atom->BeginBond(j); bond; bond = atom->NextBond(j))
          {
            _snprintf(buffer, BUFF_SIZE, "%6d", (bond->GetNbrAtom(atom))->GetIdx());
            ofs << buffer;
          }
