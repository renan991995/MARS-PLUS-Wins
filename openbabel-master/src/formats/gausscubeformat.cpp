...
//------------------------------------------------------------------------------
  bool OBGaussianCubeFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    _snprintf(buffer, BUFF_SIZE,"%5d%12.6f%12.6f%12.6f", - static_cast<signed int> (mol.NumAtoms()),
    ...
    _snprintf(buffer, BUFF_SIZE,"%5d%12.6f%12.6f%12.6f", nx,
    ...
    _snprintf(buffer, BUFF_SIZE,"%5d%12.6f%12.6f%12.6f", ny,
    ...
    _snprintf(buffer, BUFF_SIZE,"%5d%12.6f%12.6f%12.6f", nz,
    ...
    FOR_ATOMS_OF_MOL (atom, mol) {
      ...
      _snprintf(buffer, BUFF_SIZE,"%5d%12.6f%12.6f%12.6f%12.6f", atom->GetAtomicNum(),
    ...
    _snprintf(buffer, BUFF_SIZE," %5lu", (unsigned long)grids.size());
    ofs << buffer << flush;
    for (unsigned int l = 1; l <= grids.size(); ++l)
    {
      _snprintf(buffer, BUFF_SIZE," %3d", l);
      ofs << buffer << flush;
    }
    ofs << endl;
      ...
      for (int j = 0; j < ny; ++j)
      {
        for (int k = 0; k < nz; ++k)
        {
          ...
          for (unsigned int l = 0; l < grids.size(); ++l)
          {
            ...
            _snprintf(buffer, BUFF_SIZE," %12.5E", value);
            if (count % 6 == 0)
              ofs << buffer << endl;
            else
              ofs << buffer;
