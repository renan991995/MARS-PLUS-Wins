...
namespace OpenBabel {
  ...
  bool MDFFFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    {
      _snprintf(buffer, BUFF_SIZE, "%-3s ", atypes_def[i].first.c_str());
      ofs << buffer ;
    }
    ofs << endl;
    ...
    for (int i = 0; i < atypes_def.size(); i++)
    {
      _snprintf(buffer, BUFF_SIZE, "%-3u ", atypes_def[i].second);
      ofs << buffer ;
    }
    ofs << endl;
    ...
    {  
      ...
      _snprintf(buffer, BUFF_SIZE, "%-3s %26.19f %26.19f %26.19f", smb.c_str(),
