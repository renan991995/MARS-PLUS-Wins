...
namespace OpenBabel
{
  ...
  bool BGFFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    ofs << "BIOGRF 200\n";
    _snprintf(buffer, BUFF_SIZE, "DESCRP %s\n",mol.GetTitle());
    ofs << buffer;
    _snprintf(buffer, BUFF_SIZE, "REMARK BGF file created by Open Babel %s\n",BABEL_VERSION);
    ...
    for (atom = mol.BeginAtom(i);atom;atom = mol.NextAtom(i))
      {
        ...
        if (max_val == 0)
          max_val = 1;
        _snprintf(atm_sym,16,"%s%d",elmnt_typ,atom->GetIdx());
        _snprintf(buffer,BUFF_SIZE,"%6s %5d %-5s %3s %1s %5s%10.5f%10.5f%10.5f %-5s%3d%2d %8.5f\n",
                "HETATM",
                atom->GetIdx(),
                atm_sym,
                "RES",
    ...
    for (atom = mol.BeginAtom(i);atom;atom = mol.NextAtom(i))
      if (atom->GetExplicitDegree())
        {
          _snprintf(buffer,BUFF_SIZE,"CONECT%6d",atom->GetIdx());
          ofs << buffer;
          for (nbr = atom->BeginNbrAtom(j);nbr;nbr = atom->NextNbrAtom(j))
            {
              _snprintf(buffer,BUFF_SIZE,"%6d",nbr->GetIdx());
              ofs << buffer;
            }
          ofs << endl;

          _snprintf(buffer,BUFF_SIZE,"ORDER %6d",atom->GetIdx());
          ofs << buffer;
          for (nbr = atom->BeginNbrAtom(j);nbr;nbr = atom->NextNbrAtom(j))
            {
              _snprintf(buffer,BUFF_SIZE,"%6d",(*j)->GetBondOrder());
              ofs << buffer;
            }
          ofs << endl;
        }
