...
bool FenskeZmatFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
{
    ...
    for (atom = mol.BeginAtom(i);atom;atom = mol.NextAtom(i))
    {
        ...
        if (atom->GetIdx() == 1)
        {
            _snprintf(buffer, BUFF_SIZE, "%-2s  1\n",type);
            ofs << buffer;
            continue;
        }
        ...
        if (atom->GetIdx() == 2)
        {
            _snprintf(buffer, BUFF_SIZE, "%-2s%3d%6.3f\n",
                    type, a->GetIdx(), r);
            ofs << buffer;
            continue;
        }
        ...
        if (atom->GetIdx() == 3)
        {
            _snprintf(buffer, BUFF_SIZE, "%-2s%3d%6.3f%3d%8.3f\n",
        ...
        if (t < 0)
            t += 360;
        _snprintf(buffer, BUFF_SIZE, "%-2s%3d%6.3f%3d%8.3f%3d%6.1f\n",
