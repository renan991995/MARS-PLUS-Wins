...
namespace OpenBabel
{
  ...
  bool MacroModFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    char buffer[BUFF_SIZE];
    _snprintf(buffer, BUFF_SIZE, " %5d %6s      E = %7.3f KJ/mol",
    ...
    for (atom = mol.BeginAtom(i);atom;atom = mol.NextAtom(i)) {
        ...
        _snprintf(buffer, BUFF_SIZE, "%4d",type);
        ofs << buffer;
        for (nbr = atom->BeginNbrAtom(j);nbr;nbr = atom->NextNbrAtom(j))
          {
            _snprintf(buffer, BUFF_SIZE, " %5d %1d",nbr->GetIdx(),(*j)->GetBondOrder());
            ofs << buffer;
          }
        for (k=atom->GetExplicitDegree();k < 6;k++)
          {
            _snprintf(buffer, BUFF_SIZE," %5d %1d",0,0);
            ofs << buffer;
          }

        _snprintf(buffer, BUFF_SIZE, " %11.6f %11.6f %11.6f %5d %5d %8.5f \n",
