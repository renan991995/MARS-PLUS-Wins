...
namespace OpenBabel
{
  ...
  bool ZINDOFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    if (cndoStyle) {
      ...
      if (charged)
        {
          ...
          ofs << " NOP = 1 " << '\n';
          ofs << " NDT = 1 " << '\n';
          _snprintf(buffer, BUFF_SIZE, " FOP(1) =% 4d% 10.6f",
                   valenceE - 1, 1.0);
          ofs << buffer << '\n';
        }

      _snprintf(buffer, BUFF_SIZE, " NAT          %4d   NEL        %4d   MULT           1",
               mol.NumAtoms(),
               valenceE);
      ofs << buffer << '\n';
      ...
      _snprintf(buffer, BUFF_SIZE, " DYNAL(1) =     0%5d%5d    0    0 1200%5d",
    ...
    for (atom = mol.BeginAtom(i); atom; atom = mol.NextAtom(i))
      {
        _snprintf(buffer, BUFF_SIZE, "% 10.6f% 10.6f% 10.6f%5d",
                 atom->GetX(),
                 atom->GetY(),
                 atom->GetZ(),
                 atom->GetAtomicNum());
    ...
    if (!cndoStyle) { // old-style ZINDO footer for CI specification
      ...
      if (charged)
        _snprintf(buffer, BUFF_SIZE, "%5d%5d%5d%5d", 1, orbitals, orbitals, orbitals + 1);
      else
        _snprintf(buffer, BUFF_SIZE, "%5d%5d%5d", 1, orbitals, orbitals);
      ofs << buffer << '\n';
      if (charged)
        _snprintf(buffer, BUFF_SIZE, "%5d%5d%5d%5d%5d",
