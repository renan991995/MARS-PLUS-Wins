...
namespace OpenBabel
{
  ...
  bool CSSRFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    _snprintf(buffer, BUFF_SIZE, "%4d   1 %s\n",mol.NumAtoms(), mol.GetTitle());
    ...
    for(atom = mol.BeginAtom(i);atom;atom = mol.NextAtom(i))
      {
        ...
        _snprintf(buffer, BUFF_SIZE, "%4d%2s%-3d  %9.5f %9.5f %9.5f ",
                 atom->GetIdx(),
        ...
        for (nbr = atom->BeginNbrAtom(j); nbr; nbr = atom->NextNbrAtom(j))
          {
            if (bonds > 8) break;
            _snprintf(buffer, BUFF_SIZE, "%4d",nbr->GetIdx());
            ofs << buffer;
            bonds++;
          }
        for (; bonds < 8; bonds ++)
          {
            _snprintf(buffer, BUFF_SIZE, "%4d",0);
            ofs << buffer;
          }
        _snprintf(buffer, BUFF_SIZE, " %7.3f%4d", atom->GetPartialCharge(), 1);
        ofs << buffer << endl;
      }
