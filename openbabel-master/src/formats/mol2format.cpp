...
namespace OpenBabel
{
  ...
  bool MOL2Format::WriteMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    _snprintf(buffer, BUFF_SIZE," %d %d 0 0 0", mol.NumAtoms(),mol.NumBonds());
    ofs << buffer << endl;
        ...
        if (!ligandsOnly && (res = atom->GetResidue()) )
          {
            ...
            _snprintf(label,BUFF_SIZE,"%s",(char*)res->GetAtomID(atom).c_str());
            // make sure that residue name includes its number
            _snprintf(rlabel,BUFF_SIZE,"%s%d",res->GetName().c_str(), res->GetNum());
            _snprintf(rnum,BUFF_SIZE,"%d",res->GetNum());
          }

        _snprintf(buffer,BUFF_SIZE,"%7d %-6s   %9.4f %9.4f %9.4f %-5s %3s  %-8s %9.4f",
                 atom->GetIdx(),label,
    ...
    for (bond = mol.BeginBond(j);bond;bond = mol.NextBond(j))
      {
        ...
        if (bond->IsAromatic() || s1 == "O.co2" || s2 == "O.co2")
          ...
          _snprintf(label,BUFF_SIZE,"%d",bond->GetBondOrder());

        _snprintf(buffer, BUFF_SIZE,"%6d %5d %5d   %2s",
