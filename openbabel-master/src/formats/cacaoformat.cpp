...
namespace OpenBabel
{
  ...
  bool CacaoFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    ofs << buffer;
    _snprintf(buffer, BUFF_SIZE, "%3d   DIST  0  0  0\n",mol.NumAtoms());
    ofs << buffer;
    ...
    if (!mol.HasData(OBGenericDataType::UnitCell))
    ...
    else
      {
        ...
        _snprintf(buffer, BUFF_SIZE, "CELL %f,%f,%f,%f,%f,%f\n",
    ...
    for (atom = mol.BeginAtom(i);atom;atom = mol.NextAtom(i))
      {
        _snprintf(buffer,BUFF_SIZE,"%2s %7.4f, %7.4f, %7.4f\n",
  ...
  bool CacaoInternalFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    ofs << " # TITLE\n";
    _snprintf(buffer, BUFF_SIZE, "%3d  0DIST  0  0  0\n",mol.NumAtoms());
    ofs << "  EL\n";
    _snprintf(buffer, BUFF_SIZE, "0.,0.,0., %s\n",tmptype);
    ofs << buffer;
    for (i = 2; i <= mol.NumAtoms(); i++)
      {
        ...
        if (vit[i]->_tor < 0.0)
          vit[i]->_tor += 360.0;
        _snprintf(buffer, BUFF_SIZE, "%2d,%d,%2s%7.3f,%7.3f,%7.3f",
