...
namespace OpenBabel
{
  ...
  bool LMPDATFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    FOR_BONDS_OF_MOL(bond,mol)
    {
	    ...
	    {
		    _snprintf(BondString,BUFF_SIZE,
				    "%2s:%2s",
    ...
    FOR_ANGLES_OF_MOL(angle,mol)
    {
	    ...
	    {
		    _snprintf(AngleString,BUFF_SIZE,
				    "%2s:%2s:%2s",
				    OBElements::GetSymbol(b->GetAtomicNum()),
				    OBElements::GetSymbol(a->GetAtomicNum()),
    ...
    FOR_TORSIONS_OF_MOL(dihedral, mol)
    {
	    ...
	    {
		    _snprintf(DihedralString,BUFF_SIZE,
				    "%2s:%2s:%2s:%2s",
				    OBElements::GetSymbol(a->GetAtomicNum()),
				    OBElements::GetSymbol(b->GetAtomicNum()),
				    OBElements::GetSymbol(c->GetAtomicNum()),
    ...
    ofs << endl;
    ofs << "Atoms" << endl << endl;
    _snprintf(buffer,BUFF_SIZE,"#%3s %4s %4s %10s %10s %10s %10s\n",
    ...
    for(molitr=mols.begin();molitr!=mols.end();++molitr)
    {
	    ...
	    FOR_ATOMS_OF_MOL(atom,*molitr) 
	    {
		    ...
		    atomcount++;
		    ThisAtom=OBElements::GetSymbol(atom->GetAtomicNum());
		    _snprintf(buffer,BUFF_SIZE,"%-4d %4d %4d %10.5f %10.5f %10.5f %10.5f # %3s\n",
				    atomcount,molcount,
				    AtomType[ThisAtom],
				    AtomCharge[ThisAtom],
				    atom->GetX(),
