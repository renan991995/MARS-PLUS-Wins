...
bool OBMoldenFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
{
    ...
    FOR_ATOMS_OF_MOL(atom, mol)
      {
        _snprintf(buffer, BUFF_SIZE, "%2s%6d%3d%13.6f%13.6f%13.6f\n",
	...
	_snprintf(buffer, BUFF_SIZE, "%10.4f\n", frequencies[i]);
        ofs << buffer;
      }
	...
	for (unsigned int i = 0; i < vib->GetNumberOfFrequencies(); i++) {
      ...
      FOR_ATOMS_OF_MOL(atom, mol)
        {
          _snprintf(buffer, BUFF_SIZE, "%2s%13.6f%13.6f%13.6f\n",
                  OBElements::GetSymbol(atom->GetAtomicNum()),
	...
	_snprintf(buffer, BUFF_SIZE, "vibration%6d\n", mode+1);
	ofs << buffer;
        vector<vector3> lx = vib->GetLx()[mode];
	for (unsigned int i = 0; i < mol.NumAtoms(); i++) {
	  vector3 disp = lx[i];
	  _snprintf(buffer, BUFF_SIZE, "%12.6f%13.6f%13.6f\n",
