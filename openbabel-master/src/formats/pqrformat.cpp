...
namespace OpenBabel
{
  ...
  bool PQRFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    if (!pConv->IsLast() || pConv->GetOutputIndex() > 1)
      ...
      _snprintf(buffer, BUFF_SIZE, "MODEL %8d", model_num);
      ofs << buffer << endl;
      }

    if (strlen(mol.GetTitle()) > 0)
      _snprintf(buffer, BUFF_SIZE, "COMPND    %s ",mol.GetTitle());
    else
      _snprintf(buffer, BUFF_SIZE, "COMPND    UNNAMED");
    ofs << buffer << endl;

    _snprintf(buffer, BUFF_SIZE, "AUTHOR    GENERATED BY OPEN BABEL %s",BABEL_VERSION);
    ofs << buffer << endl;
    ...
    for (i = 1; i <= mol.NumAtoms(); i++)
      {
        ...
        if (strlen(type_name) > 1)
        ...
        else
          {
            ...
            _snprintf(type_name, sizeof(type_name), " %-3s", tmp);
          }

        if ((res = atom->GetResidue()) != nullptr)
          {
            het = res->IsHetAtom(atom);
            _snprintf(the_res,4,"%s",(char*)res->GetName().c_str());
            _snprintf(type_name,5,"%s",(char*)res->GetAtomID(atom).c_str());
                ...
                if (strlen(type_name) < 4)
                  {
                    ...
                    _snprintf(padded_name, sizeof(padded_name), " %-3s", tmp);
          ...
          {
            strcpy(the_res,"UNK");
            _snprintf(padded_name,sizeof(padded_name), "%s",type_name);
            strncpy(type_name,padded_name,4);
            type_name[4] = '\0';
            res_num = 1;
          }
        ...
        //_snprintf(buffer, BUFF_SIZE, "%s%5d %-4s %-3s %c%4d    %8.3f%8.3f%8.3f  1.00  0.00          %2s  \n",
        _snprintf(buffer, BUFF_SIZE, "%s%5d %-4s %-3s %c%4d    %8.3f%8.3f%8.3f %11.8f%8.3f %2s  \n",
                 het?"HETATM":"ATOM  ",
                 i,
                 type_name,
                 the_res,
    ...
    for (i = 1; i <= mol.NumAtoms(); i ++)
      {
        ...
        _snprintf(buffer, BUFF_SIZE, "CONECT%5d", i);
        ofs << buffer;
        // Write out up to 4 real bonds per line PR#1711154
        int currentValence = 0;
        for (nbr = atom->BeginNbrAtom(k);nbr;nbr = atom->NextNbrAtom(k))
          {
            _snprintf(buffer, BUFF_SIZE, "%5d", nbr->GetIdx());
            ofs << buffer;
        ...
        for (int count = 0; count < (4 - remainingValence); count++) {
          _snprintf(buffer, BUFF_SIZE, "     ");
          ofs << buffer;
        }
        ofs << "                                       \n";
      }

    _snprintf(buffer, BUFF_SIZE, "MASTER        0    0    0    0    0    0    0    0 ");
    ofs << buffer;
    _snprintf(buffer, BUFF_SIZE, "%4d    0 %4d    0\n",mol.NumAtoms(),mol.NumAtoms());
    ofs << buffer;
