...
namespace OpenBabel
{
 ...
 bool mmCIFFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
 {
   ...
   if (id.empty())
     {
     _snprintf(buffer, BUFF_SIZE, "T%lu", (unsigned long)time(nullptr));
     id.assign(buffer);
     }
   ofs << "# --------------------------------------------------------------------------" << endl;
   ofs << "#" << endl;
   ...
   for (OBAtomIterator atom_x = pmol->BeginAtoms(), atom_y = pmol->EndAtoms(); atom_x != atom_y; ++ atom_x, ++ site_id)
     ...
     if (has_residues)
       ...
       if (atomname.empty())
         {
         _snprintf(buffer, BUFF_SIZE, "%s%lu", OBElements::GetSymbol(atom->GetAtomicNum()), (unsigned long)site_id);
