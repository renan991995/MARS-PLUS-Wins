		...
		FOR_ATOMS_OF_MOL(atom, mol)
      {
        ...
        c = vic[atom->GetIdx()]->_c;
				...
				if (atom->GetIsotope() != 0) {
					_snprintf(buffer, BUFF_SIZE, "(Iso=%d)", atom->GetIsotope());
					type += buffer;
				}
          ...
          {
					case 1:
            _snprintf(buffer, BUFF_SIZE, "%-s\n",type.c_str());
            ofs << buffer;
            continue;
            break;

					case 2:
            _snprintf(buffer, BUFF_SIZE, "%-s  %d  r%d\n",
            ...
            break;
					...
					case 3:
            _snprintf(buffer, BUFF_SIZE, "%-s  %d  r%d  %d  a%d\n",
            ...
            break;
					...
					default:
            _snprintf(buffer, BUFF_SIZE, "%-s  %d  r%d  %d  a%d  %d  d%d\n",
                     type.c_str(), a->GetIdx(), atom->GetIdx(),
        ...
        t = vic[atom->GetIdx()]->_tor;
				...
				switch(atom->GetIdx())
          {
            ...
            break;
					...
					case 2:
            _snprintf(buffer, BUFF_SIZE, "r2= %6.4f\n", r);
            ofs << buffer;
            continue;
            break;

					case 3:
            _snprintf(buffer, BUFF_SIZE, "r3= %6.4f\na3= %6.2f\n", r, w);
            ofs << buffer;
            continue;
            break;

					default:
            _snprintf(buffer, BUFF_SIZE, "r%d= %6.4f\na%d= %6.2f\nd%d= %6.2f\n",
