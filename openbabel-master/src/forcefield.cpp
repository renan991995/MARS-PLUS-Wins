  ...
  **/
  ...
  void OBForceField::PrintVelocities()
  {
    IF_OBFF_LOGLVL_LOW {
      ...
      FOR_ATOMS_OF_MOL (a, _mol) {
        _snprintf(_logbuf, BUFF_SIZE, "%d\t<%8.3f, %8.3f, %8.3f>\n", a->GetIdx(), _velocityPtr[a->GetIdx()],
  ...
  int OBForceField::SystematicRotorSearchInitialize(unsigned int geomSteps, bool sampleRingBonds)
  {
    ...
    IF_OBFF_LOGLVL_LOW {
      ...
      _snprintf(_logbuf, BUFF_SIZE, "  NUMBER OF ROTATABLE BONDS: %lu\n", (unsigned long)rl.Size());
      OBFFLog(_logbuf);
      ...
      _snprintf(_logbuf, BUFF_SIZE, "  NUMBER OF POSSIBLE ROTAMERS: %lu\n", combinations);
      OBFFLog(_logbuf);
    }
    ...
    IF_OBFF_LOGLVL_LOW {
      _snprintf(_logbuf, BUFF_SIZE, "  GENERATED %d CONFORMERS\n\n", _mol.NumConformers());
      OBFFLog(_logbuf);
  ...
  bool OBForceField::RandomRotorSearchNextConformer(unsigned int geomSteps)
  {
    ...
    IF_OBFF_LOGLVL_LOW {
      _snprintf(_logbuf, BUFF_SIZE, "   %3d      %8.3f\n", (_current_conformer + 1), _energies[_current_conformer]);
      OBFFLog(_logbuf);
    }
                                         ...
                                         bool sampleRingBonds)
  {
    ...
    IF_OBFF_LOGLVL_HIGH {
      ...
      for (unsigned int i = 1; i < rotorWeights.size() - 1; ++i) {
        _snprintf(_logbuf, BUFF_SIZE, " Weight: %d", i);
        OBFFLog(_logbuf);
        for (unsigned int j = 0; j < rotorWeights[i].size(); ++j) {
          _snprintf(_logbuf, BUFF_SIZE, " %8.3f", rotorWeights[i][j]);
          OBFFLog(_logbuf);
        }
        OBFFLog("\n");
      }
  ...
  void OBForceField::DistanceGeometry()
  {
    ...
    IF_OBFF_LOGLVL_LOW {
      ...
      for (i=0; i<N; i++) {
        OBFFLog("[");
        for (j=0; j<N; j++) {
          _snprintf(_logbuf, BUFF_SIZE, " %8.4f ", matrix[i][j]);
          OBFFLog(_logbuf);
        }
        OBFFLog("]\n");
      }
    ...
    IF_OBFF_LOGLVL_LOW {
      ...
      for (i=0; i<N; i++) {
        OBFFLog("[");
        for (j=0; j<N; j++) {
          _snprintf(_logbuf, BUFF_SIZE, " %8.4f ", G[i][j]);
          OBFFLog(_logbuf);
        }
        OBFFLog("]\n");
      }
    ...
    IF_OBFF_LOGLVL_LOW {
      ...
      for (i=0; i<N; i++) {
        _snprintf(_logbuf, BUFF_SIZE, "%8.4f ", eigenvalues[i]);
        OBFFLog(_logbuf);
      }
      OBFFLog("\n");
      ...
      for (i=0; i<N; i++) {
        OBFFLog("[");
        for (j=0; j<N; j++) {
          _snprintf(_logbuf, BUFF_SIZE, " %8.4f ", eigenvectors[i][j]);
          OBFFLog(_logbuf);
        }
        OBFFLog("]\n");
      }
  ...
  void OBForceField::UpdatePairsSimple()
  {
    ...
    /*
      IF_OBFF_LOGLVL_LOW {
      _snprintf(_logbuf, BUFF_SIZE, "UPDATE VDW PAIRS: %d --> %d (VDW), %d (ELE) \n", i+1,
  ...
  void OBForceField::ValidateSteepestDescent(int steps)
  {
    ...
    IF_OBFF_LOGLVL_LOW {
      ...
      _snprintf(_logbuf, BUFF_SIZE, "STEPS = %d\n\n",  steps);
      OBFFLog(_logbuf);
    ...
    for (int i = 1; i <= steps; i++) {
      ...
      IF_OBFF_LOGLVL_LOW {
        _snprintf(_logbuf, BUFF_SIZE, " %4d    %8.3f    %8.3f\n", i, e_n2, e_n1);
        OBFFLog(_logbuf);
      }
  ...
  void OBForceField::SteepestDescentInitialize(int steps, double econv, int method)
  {
    ...
    IF_OBFF_LOGLVL_LOW {
      ...
      OBFFLog("STEP n       E(n)         E(n-1)    \n");
      OBFFLog("------------------------------------\n");
      _snprintf(_logbuf, BUFF_SIZE, " %4d    %8.3f      ----\n", _cstep, _e_n1);
      OBFFLog(_logbuf);
    }
  ...
  bool OBForceField::SteepestDescentTakeNSteps(int n)
  {
    ...
    for (int i = 1; i <= n; i++) {
      ...
      IF_OBFF_LOGLVL_LOW {
        if (_cstep % 10 == 0) {
          _snprintf(_logbuf, BUFF_SIZE, " %4d    %8.5f    %8.5f\n", _cstep, e_n2, _e_n1);
          OBFFLog(_logbuf);
        }
      }
