  ...
  void OBChainsParser::SetResidueInformation(OBMol &mol, bool nukeSingleResidue)
  {
    ...
    for ( unsigned int i = 0 ; i < numAtoms ; i++ ) {
      ...
      if (atomids[i] == -1) {
        ...
        if (hcounts[i]) {
          _snprintf(buffer, BUFF_SIZE, "H%.2s%c", ChainsAtomName[atomids[i]]+2, hcounts[i]+'0');
          if (buffer[1] == ' ') {
            buffer[1] = buffer[3];
            buffer[2] = '\0';
          }
          ...
          _snprintf(buffer, BUFF_SIZE, "H%.2s", ChainsAtomName[atomids[i]]+2);
        }
      } else
        _snprintf(buffer, BUFF_SIZE, "%.4s", ChainsAtomName[atomids[i]]);
